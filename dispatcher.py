...
def dispatcher(config_d: dict):
    ...
    data_analysis_dir = config_d['data_analysis_dir']
    new_dir = check_newdir(data_analysis_dir)
    if new_dir:
        ...
        logging.info("{}中没有满足条件的、需要分析的目录".format(data_analysis_dir))
        sys.exit()
...
def main():
    ...
    elif os.path.exists("{0}/../config/configuration.json".format(script_dir)):
        config_file = "{0}/../config/configuration.json".format(script_dir)
    else:
